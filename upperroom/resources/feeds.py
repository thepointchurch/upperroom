    ...
    def add_root_elements(self, handler):  # NOQA: C901
        ...
        if self.feed["is_podcast"]:
            ...
            handler.addQuickElement(
                "itunes:email", self.feed["owner_email"] if "owner_email" in self.feed else settings.WEBMASTER_EMAIL
            )
            ...
            handler.addQuickElement("itunes:image", attrs={"href": urllib.parse.urljoin(static_root, image_url)})
...
class ResourceFeedRSS(Feed):
    ...
    def get_object(self, request, slug):  # pylint: disable=arguments-differ
        self.object = get_object_or_404(ResourceFeed, slug=slug)  # pylint: disable=attribute-defined-outside-init
    ...
    def items(self, obj):
        resources = Resource.published_objects.filter(is_private=False).filter(tags__in=obj.tags.all())
